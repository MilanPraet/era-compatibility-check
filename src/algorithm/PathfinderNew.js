...
export class PathFinder extends EventEmitter {
    ...
    async aStar({ from, to, NG }) {
                    ...
                    if (nextNode.lngLat) {
                        ...
                        while (geoDist === null) {
                            ...
                            if (nextNextNode) {
                                ...
                                } else if (nextNextNode.nextNodes.size > 0) {
                                    hereId = nextId;
        ...
        if (dest) {
            ...
            path.nodes.unshift({ id: node.from, length: NG.nodes.get(node.from).length, lngLat: from.lngLat });
    ...
    async fetchAbstractionTile({ coords, NG }) {
        const tileUrl = `${this.abstTilesBaseURI}/${this.zoom}/${Utils.longLat2Tile(coords, this.zoom)}`
        let res = null;
